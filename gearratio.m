...
stallTorque = [123 0]; %[N*m] stall torque at output
noLoadSpeed = [0 41.888]; %[rad/s] output no load speed 
n = 0.98; %gearing efficiency 
P_in = 600; %[W] Input power from motor to transmission system 
P_transmitted = n*P_in; %[W] Output(transmitted) power throughout transmission system 

%Torque/Speed Curve 
plot(noLoadSpeed,stallTorque);
...
P_out = [-Ts_out/Wn_out Ts_out -P_transmitted]; %vector for polynomial Power(w)=-(Ts/Wn)*w^2 + Ts*Wn that represents power curve in terms of speed
roots_Speed=roots(P_out); %output operating speed @588W is w2=5.5036 and w1=36.3844 (from workspace)
